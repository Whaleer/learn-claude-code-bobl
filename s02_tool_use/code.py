...
# 把 .env 文件里的变量加载进当前 Python 进程的环境变量里
load_dotenv(override = True)

# 获取 .env 中的环境变量 ANTHROPIC_BASE_URL
# 如何这个环境变量存在，那么在当前 Python 进程的环境变量里删除：ANTHROPIC_AUTH_TOKEN
...
SYSTEM = f"you are a coding agent at {os.getcwd()}. Use bash to solve tasks. Act, dont explain."
...
def run_bash(command: str) -> str:
    dangerous = ["rm -rm /", "sudo", "shutdown", "reboot", "> /dev/"]
